...
- [ ] **AICM 1.0.3 source spreadsheet** (`control/cloudsecurityalliance.org/aicm/1.0.3/`) — the original XLSX was never archived. `s3://dataset-public-laws-regulations-standards/control/cloudsecurityalliance.org/aicm/` holds only the 1.1.0 workbook, and the file is not on local disk either, so **the 1.0.3 extraction is not currently reproducible from source** — `1.0.3/scripts/parse_aicm.py` points at a bundle directory (`AICMv1.0.3+AI_CAIQv1.0.2-spreadsheet_bundle-generated_at_2025_11_10/`) that exists nowhere. The committed JSON is intact and 1.0.3 remains the only release carrying the NIST AI 600-1 mappings that 1.1.0 withdrew, so this matters. **Plan:** re-download the v1.0.3 bundle from CSA (superseded artifacts are not always kept on the artifact page — may need CSA-internal access or an archived copy), drop the XLSX at the SecID-canonical path, and run `tools-resources/utils/sync_originals_to_s3.sh`. Same applies to the AI-CAIQ 1.0.2 workbook under `aicm-caiq/1.0.2/`. Noted 2026-07-31.
