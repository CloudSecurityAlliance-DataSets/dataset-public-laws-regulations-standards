...
def render(rows, removed, old_version, new_version, counts):
    ...
    w(f"The same data, with more of it, is in")
    w(f"[`aicm-{new_version}-changelog.json`](aicm-{new_version}-changelog.json) — use that")
    w("rather than parsing this table. It carries the numeric specification similarity and")
    w("the structured `id_reuse` object, both of which are flattened into prose here.\n")
    w("```jsonc")
    w("{")
    w('  "specification_name": "AI Controls Matrix",')
    w(f'  "version":          "{new_version}",   // the release this changelog describes')
    w(f'  "compared_against": "{old_version}",   // the release it is diffed against')
    w('  "summary":  { /* counts */ },   // the figures in "The short version" above')
    w('  "controls": [ /* ... */ ],      // one entry per control in this release')
    w('  "removed":  [ /* ... */ ]       // controls with no successor in this release')
    w("}")
    w("```\n")
    w("Each entry in `controls`:\n")
    w("| Field | Type | Meaning |")
    w("|---|---|---|")
    w(f"| `id` | string | Control identifier in {new_version} |")
    w("| `domain` | string | Domain code, e.g. `LOG`. Note `A&A` and `I&S` contain an ampersand |")
    w(f"| `title` | string | Control title in {new_version} |")
    w(f"| `previous_id` | string \\| null | Identifier in {old_version}; `null` if the control is new |")
    w(f"| `previous_title` | string \\| null | Title in {old_version}; `null` if the control is new |")
    w("| `status` | string | `unchanged`, `changed`, or `new` |")
    w("| `changes` | array | Any of `renumbered`, `retitled`, `rewritten`, `edited`. Empty when `unchanged` or `new` |")
    w(f"| `spec_similarity` | number \\| null | 0-1 similarity of the requirement text against {old_version}. `null` for new controls. Below {COSMETIC_MIN} is classed `rewritten` |")
    w("| `id_reuse` | object \\| null | Non-null when **this identifier designated a different control** before. See below |")
    w("| `heuristic_match` | boolean | True where the old/new pairing was inferred rather than certain |")
    w("")
    w("`id_reuse`, when present:\n")
    w("| Field | Meaning |")
    w("|---|---|")
    w(f"| `previously_designated` | Title this identifier carried in {old_version} |")
    w(f"| `that_control_is_now` | Where that control lives in {new_version}, or `null` if it was removed |")
    w("")
    w("Each entry in `removed` carries `id`, `domain`, `title`, the full `specification`")
    w("text, and `id_reassigned_to` — the title the identifier now points at, or `null` if")
    w("the identifier is unused.\n")
    w("Finding every control whose identifier changed meaning is therefore:\n")
    w("```python")
    w("import json")
    w(f'changelog = json.load(open("aicm-{new_version}-changelog.json"))')
    w('repointed = [c for c in changelog["controls"] if c["id_reuse"]]')
    w("```\n")
    w("## Controls\n")
