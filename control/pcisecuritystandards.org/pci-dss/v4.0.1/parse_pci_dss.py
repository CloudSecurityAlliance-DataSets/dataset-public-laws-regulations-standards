...
Input:  pci-dss-v4.0.1.json  (marker output, --output_format json)
Output: pci-dss-v4.0.1-requirements.csv
        pci-dss-v4.0.1-requirements.json
...
def collect_requirement_tables(doc):
    ...
    for n in walk_blocks(doc):
        if n.get("block_type") != "Table":
            continue
        html = n.get("html") or ""
        if "Defined Approach Requirements" in html:
            out.append(n)
    return out
...
def extract_requirement_id(col1_text):
    ...
    The requirement ID appears as a standalone token like '1.1.1' or '8.3.1'.
    Multiple IDs may appear in cross-references; we want the first one that
    looks like 'the requirement this table describes' (typically appears near
    the start of the cell).
    """
    if not col1_text:
        return None
    # Prefer IDs that appear on their own line or after a newline
...
def parse_table(table_node):
    ...
    for i, r in enumerate(rows):
        if r.find(string=re.compile("Defined Approach Requirements")):
            header_idx = i
            break
...
def main():
    ...
    input_path = here / "pci-dss-v4.0.1.json"
    csv_path = here / "pci-dss-v4.0.1-requirements.csv"
    json_path = here / "pci-dss-v4.0.1-requirements.json"
