...
Output:
  800-63a-sections.{csv,json}  — one row per numbered section
"""
import csv
import json
import re
...
def main():
    md = open("800-63a.md", encoding="utf-8").read()
    md = re.sub(r"\{\d+\}-+", "", md)
    ...
    with open("800-63a-sections.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields, quoting=csv.QUOTE_ALL)
        w.writeheader()
        w.writerows(final)
    with open("800-63a-sections.json", "w", encoding="utf-8") as f:
